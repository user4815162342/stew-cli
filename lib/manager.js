...
module.exports.startup = function(options,cb) {
    ...
    if (typeof projectPath !== "undefined") {
        ...
        require(command)(projectPath,true).run(function(err,result) {
...
var requireRelativeDoc = module.exports.requireRelativeDoc = function(p,cb) {
    if (typeof p === "undefined") {
        ...
        // NOTE: This form should be deprecated, since the other mechanism
        // works better.
...
var changeToFirstChild = module.exports.changeToFirstChild = function(filter,cb) {
    // Needs to be relative to the current doc, so we need to get that
    // first.
    requireWorkingDoc(function(err,doc) {
        if (err) {
        ...
        doc.listDocs(filter,function(err,children) {
            if (err) {
                return cb(err);
            }
            if (children.length) {
...
var commands = {
    ...
    goto: engine.asyncCommand(changeWorkingDoc,
            function(doc) {
                if (typeof doc === "undefined") {
                    return [path.sep];
    ...
    next: engine.asyncCommand(changeWorkingDocBy.bind(null,1),function(filter) { 
    ...
    previous: engine.asyncCommand(changeWorkingDocBy.bind(null,-1),function(filter) { 
        if (typeof filter === "string") {
            filter = filters.parse(filter);
        } 
        if ((typeof filter === "function") ||
    ...
    down: engine.asyncCommand(changeToFirstChild,function(filter) { 
        if (typeof filter === "string") {
            filter = filters.parse(filter);
        } 
        if ((typeof filter === "function") ||
    ...
    up: engine.asyncCommand(changeWorkingDoc.bind(null,".."),function() { return [] }),
            
    // TODO: This is deprecated now that commands which need a relative
    // doc accept glob pattern strings.
    glob: function(match) {
