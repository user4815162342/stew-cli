...
module.exports.startup = function(options) {
    ...
    if (typeof projectPath !== "undefined") {
        ...
        // is resolved.
        return require(command)(projectPath,true).runPromise().catch(
            function(err) {
...
var requireProject = module.exports.requireProject = function() {
    if (project) {
        ...
        return require("./commands/open")(process.cwd()).runPromise();
    }
}
...
var openWorkingDoc = module.exports.openWorkingDoc = function(cb) {
    if (typeof project !== "undefined") {
        ...
        var getDoc = Q.nbind(root.getDoc,root);
        return getDoc(workingDoc).then(function(doc) {
...
var requireRelativeDoc = module.exports.requireRelativeDoc = function(p) {
    if (typeof p === "undefined") {
        ...
        return requireWorkingDoc().then(function(doc) {
            ...
            var matchDocs = Q.nbind(doc.matchDocs,doc);
            return matchDocs(p).then(function(docs) {
                if (docs.length == 0) {
...
var changeWorkingDoc = module.exports.changeWorkingDoc = function(p) {
    requireRelativeDoc(p).then(function(doc) {
        setWorkingDoc(doc);
    });
}
    ...
    if (delta === 0) {
        ...
        return requireWorkingDoc().then(function(doc) {
            ...
            if (doc.path() === path.sep) {
                ...
                var getDoc = Q.nbind(project.root().getDoc,project.root());
                return getDoc(parentPath).then(function(parent) {
                    ...
                    var includeThisFilter = filter && function(filteringDoc,cb) {
                    ...
                    var listDocs = Q.nbind(parent.listDocs,parent);
                    return listDocs(includeThisFilter).then(function(siblings) {
                        var idx = -1;
...
var changeToFirstChild = module.exports.changeToFirstChild = function(filter) {
    ...
    return requireWorkingDoc().then(function(doc) {
        ...
        var listDocs = Q.nbind(doc.listDocs,doc);
        return listDocs(filter).then(function(children) {
            if (children.length) {
...
module.exports.edit = function(path,taskDone) {
    ...
    if (project) {
        var stew = Q.nbind(project.stew,project);
        return stew().then(function(stew) {
