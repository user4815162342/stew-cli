 ...
 * from settings.
 * */
module.exports.startup = function(cb) {
    // if called, then we are running in the REPL, and settings
    // should be stored.
    inREPL = true;
    
    // initialize the project that was open the last time.
    var projectPath = settings.globals.get("project");
    ...
    if (project !== p) {
        ...
        if (inREPL) {
            ...
            if (typeof p === "undefined") {
                ...
                workingDoc = settings.project(p.basePath()).get("working-doc","");
...
var requireRelativeDoc = module.exports.requireRelativeDoc = function(p,cb) {
    if (typeof p === "undefined") {
        ...
        // should be a string, RegExp or Glob if validation is working correctly, might
        ...
        requireWorkingDocPath(function(err,docPath,project) {
            ...
            if (typeof docPath !== "undefined") {
                if ((p instanceof Glob) || (p instanceof RegExp)) {
                    ...
                    p = path.resolve(docPath,p);
                    project.root().getDoc(p,cb);
                }
            } else {
...
var changeWorkingDocBy = module.exports.changeWorkingDocBy = function(delta,cb) {
    if (delta === 0) {
        ...
        requireWorkingDoc(function(err,doc) {
            ...
            if (doc.path() === "/") {
                ...
                project.root().getDoc(parentPath,function(err,parent) {
                    ...
                    parent.listDocs(function(err,siblings) {
                        if (err) {
                            return cb(err);
                        }
                        var idx = -1;
...
var commands = {
    ...
    ndoc: engine.asyncCommand(changeWorkingDocBy.bind(null,1),function() { return [] }),

    pdoc: engine.asyncCommand(changeWorkingDocBy.bind(null,-1),function() { return [] }),
    
    cdoc: engine.asyncCommand(changeToFirstChild,function() { return [] }),
