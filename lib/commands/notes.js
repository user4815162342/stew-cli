...
module.exports = engine.promiseCommand(function(doc,ext,cb) {
    return manager.requireRelativeDoc(doc).then(function(doc) {
        if (typeof doc === "undefined") {
            throw new engine.CommandError("Not in a stew project.");
        }
        if (doc.ensureNotes) {
            return engine.callbackToPromise(doc.ensureNotes,doc)(ext).then(function(path) {
                return manager.edit(path);
            },function(err) {
                if (err.code) {
                    switch (err.code) {
                        case "CAN'T ENSURE ATTACHMENT":
